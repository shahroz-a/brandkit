...
type Command = "init" | "generate" | "export" | "build" | "sync" | "doctor" | "validate" | "batch" | "help";
...
main().catch((error: unknown) => {
  console.error(`brandkit: ${error instanceof Error ? error.message : "Unknown error"}`);
  process.exitCode = 1;
});
...
async function init(root: string): Promise<void> {
  ...
  await writeFile(target, `${JSON.stringify(DEFAULT_BRAND_TOKENS, null, 2)}\n`, "utf8");
...
async function exportZip(root: string): Promise<void> {
  ...
  console.log(`Exported ${assets.length} assets to ${path.relative(root, zipPath)}`);
}

async function sync(root: string): Promise<void> {
  const tokens = await readBrand(root);
  const assets = generateBrandAssets(tokens).filter((asset) =>
    ["favicon.svg", "apple-touch-icon.svg", "android-chrome-192x192.svg", "android-chrome-512x512.svg", "site.webmanifest", "og-image.svg", "twitter-card.svg", "robots.txt"].includes(asset.filename)
  );
...
async function doctor(root: string): Promise<void> {
  ...
  console.log(`Score: ${Math.max(0, validation.score - projectChecks.filter((check) => check.severity !== "info").length * 6)}/100`);

  if (!validation.ok || projectChecks.some((check) => check.severity === "error")) {
    process.exitCode = 1;
  }
}
...
async function validate(root: string): Promise<void> {
  ...
  const validation = validateBrandTokens(tokens);
  printValidation(validation.issues);
  console.log(validation.ok ? `Valid brand kit. Score: ${validation.score}/100` : `Brand kit needs attention. Score: ${validation.score}/100`);
  process.exitCode = validation.ok ? 0 : 1;
}

async function batch(root: string, filePath: string | undefined): Promise<void> {
  if (!filePath) {
  ...
  for (const row of result.rows) {
    ...
    await writeAssets(path.join(outputRoot, slugify(tokens.name)), generateBrandAssets(tokens));
  }

  console.log(`Generated ${result.rows.length} brand kits in ${path.relative(root, outputRoot)}/`);
}
...
async function readBrand(root: string): Promise<BrandTokens> {
  ...
  try {
    const parsed = JSON.parse(await readFile(target, "utf8")) as Partial<BrandTokens>;
    return normalizeBrandTokens(parsed);
  } catch (error) {
    throw new Error(`Unable to read brand.json: ${error instanceof Error ? error.message : "Invalid JSON"}`);
  }
}

async function writeAssets(outputDir: string, assets: BrandAsset[]): Promise<void> {
  await mkdir(outputDir, { recursive: true });
  await Promise.all(assets.map((asset) => writeFile(path.join(outputDir, asset.filename), asset.content, "utf8")));
}
...
function printProjectChecks(checks: ProjectCheck[]): void {
  ...
  for (const check of checks) {
    console.log(`- ${check.severity.toUpperCase()} ${check.file}: ${check.message}`);
  }
}
